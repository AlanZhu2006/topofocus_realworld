...
def combined_sensor_recovery_health(
    detail: str = "optimizer_status=skipped_imu_invalid",
) -> RobotHealth:
    return RobotHealth.model_validate({
        "safety_state": "HOLD",
        ...
        "collision_avoidance_ready": False,
        "motor_controller_ready": True,
        "detail": f"{detail}; occupancy_age=5.633s/5.000s",
    })


def odometry_recovery_health(
    detail: str = "slam_optimizer_imu_valid",
) -> RobotHealth:
    return RobotHealth.model_validate({
        "safety_state": "HOLD",
        "localization_state": "LOST",
        ...
        "collision_avoidance_ready": True,
        "motor_controller_ready": True,
        "detail": f"{detail}; odom_age=5.307s/5.000s",
    })
...
def test_same_leg_renewal_accepts_zeroed_combined_sensor_recovery(
    observation_factory,
    recovery_reason,
):
    observations, registry, digests, now = ready_registries(
    ...
    for index, decision in enumerate(first.decisions):
        raw = make_event(
            decision,
            now=now + 500_000_000,
            event_id=f"combined-recovery-{index}",
        ).model_dump(mode="json")
        if index == 0:
            raw["status"] = "ACCEPTED"
            raw["reason_code"] = recovery_reason
    ...
    observations.accept_heartbeat(
        "robot-0",
        combined_sensor_recovery_health(),
        now + 750_000_000,
        now_ns=now + 750_000_000,
    )
...
@pytest.mark.parametrize(
    ...
    (
        ("optimizer_status=failed", True, True),
        ("optimizer_status=skipped_imu_invalid", False, True),
        ("optimizer_status=skipped_imu_invalid", True, False),
    ),
)
def test_combined_sensor_renewal_rejects_nonexact_contract(
    observation_factory,
    detail,
    zero_confirmed,
    motor_ready,
    ...
    for index, decision in enumerate(first.decisions):
        raw = make_event(
            decision,
            now=now + 500_000_000,
            event_id=f"combined-invalid-{index}",
        ).model_dump(mode="json")
        if index == 0:
            raw["status"] = "ACCEPTED"
            raw["reason_code"] = "LOCAL_SENSOR_RECOVERY_WAIT"
    ...
    health = combined_sensor_recovery_health(detail).model_copy(
        update={"motor_controller_ready": motor_ready}
    )
    observations.accept_heartbeat(
        "robot-0",
        health,
...
def test_same_leg_renewal_accepts_zeroed_odometry_recovery(
    observation_factory,
):
    observations, registry, digests, now = ready_registries(
        observation_factory
    ...
    for index, decision in enumerate(first.decisions):
        raw = make_event(
            decision,
            now=now + 500_000_000,
            event_id=f"odometry-recovery-{index}",
        ).model_dump(mode="json")
        if index == 0:
            raw["status"] = "ACCEPTED"
...
@pytest.mark.parametrize(
    ...
    (
        ("optimizer_status=failed", True, True),
        ("slam_optimizer_imu_valid", False, True),
        ("slam_optimizer_imu_valid", True, False),
    ),
)
def test_odometry_recovery_renewal_rejects_nonexact_contract(
    observation_factory,
    detail,
    zero_confirmed,
    motor_ready,
    ...
    for index, decision in enumerate(first.decisions):
        raw = make_event(
            decision,
            now=now + 500_000_000,
            event_id=f"odometry-invalid-{index}",
        ).model_dump(mode="json")
        if index == 0:
            raw["status"] = "ACCEPTED"
            raw["reason_code"] = "LOCAL_ODOMETRY_RECOVERY_WAIT"
    ...
    observations.accept_heartbeat(
        "robot-0",
        odometry_recovery_health(detail).model_copy(
            update={"motor_controller_ready": motor_ready}
        ),
        now + 750_000_000,
        now_ns=now + 750_000_000,
