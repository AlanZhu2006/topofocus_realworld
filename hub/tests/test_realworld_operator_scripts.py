...
def test_oneclick_is_session_bound_and_has_no_forensic_bypass():
    ...
    assert "FOCUS_WSJ_REACHABILITY_CLEARANCE_M:-0.20" in wsj
    assert "FOCUS_WSJ_START_SNAP_RADIUS_M:-0.35" in wsj
...
def test_live_arming_precedes_continuous_runner_and_has_exit_disarm():
    ...
    assert 'default=0.20' in runner
    assert 'default=0.35' in runner
    assert "robot-0 frontier start snap radius must equal" in runner
