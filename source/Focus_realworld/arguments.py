...
def get_args():
    ...
    parser.add_argument('--enable_pruning', action='store_true', default=False,
                        help='Enable vision token pruning (Step 7). '
                             'When enabled, the Decision VLM call uses R+K tokens '
                             'instead of 1600. Default: disabled for safe A/B testing.')

    # Checkpoint / resume
    parser.add_argument('--auto_resume', action='store_true', default=False,
                        help='Auto-resume from checkpoint if one exists for this experiment. '
                             'Metrics and episode progress are restored.')

    # Reduce file output
    parser.add_argument('--no_dod_logs', action='store_true', default=False,
                        help='Skip vlm_decisions, attention_dod, active_patches log files to save disk.')

    # Parallel evaluation (per-scene)
    parser.add_argument('--scene_id', type=str, default=None,
                        help='Run only episodes from this scene (e.g., "TEEsavR23oF"). '
                             'Used for parallel evaluation across scenes.')

    # Run mode for easy A/B switching
    parser.add_argument('--run_mode', type=str, default='pruned',
                        choices=['pruned', 'baseline', 'no_cot'],
                        help='pruned: enable_pruning + no CoT + no caption (current). '
                             'baseline: original CoT + caption, no pruning. '
                             'no_cot: no CoT + no caption, no pruning.')
