...
logger.add("server_debug.log", rotation="10 MB", level="DEBUG")
...
from attention_hooks import AttentionCaptureHooks
from vision_pruning import install_pruning, set_pruning_params, clear_pruning_params, get_pruning_state
from fastv_pruning import install_fastv, configure_fastv_from_env, fastv_status
from prumerge_pruning import install_prumerge, configure_prumerge_from_env, prumerge_status, prumerge_enabled

MODEL_PATH = 'THUDM/glm-4v-9b'
DEVICE = 'cuda:0' if torch.cuda.is_available() else 'cpu'

# Global: initialized after model loading in __main__
attn_hooks = None
...
class ChatMessageResponse(BaseModel):
    ...
    attention_index_mapping: Optional[dict] = None
...
class ChatCompletionRequest(BaseModel):
    ...
    # Step 7: Vision pruning parameters
    active_patch_indices: Optional[List[int]] = None
    room_patch_groups: Optional[dict] = None
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    gen_params = dict(
        ...
        active_patch_indices=request.active_patch_indices,
        room_patch_groups=request.room_patch_groups,
    )

    rsp = request.messages[-1].return_string_probabilities
    if rsp and rsp.startswith("[") and rsp.endswith("]"):
        # Generic parser: "[A, B, C]" -> ['A', 'B', 'C'], "[Yes, No]" -> ['Yes', 'No'], etc.
        parsed_probs = [s.strip() for s in rsp[1:-1].split(",")]
        gen_params = dict(
        messages=request.messages,
        temperature=request.temperature,
        top_p=request.top_p,
        ...
        echo=False,
        stream=request.stream,
        return_string_probabilities=parsed_probs,
        active_patch_indices=request.active_patch_indices,
        room_patch_groups=request.room_patch_groups,
        )
    elif not rsp:
    ...
    message = ChatMessageResponse(
        ...
        attention_index_mapping=response.get("attention_index_mapping"),
    )
    logger.debug(f"==== message ====\ncontent={message.content!r}  scores={message.scores}")
...
async def predict(model_id: str, params: dict):
    ...
    for new_response in generate_stream_cogvlm(model, tokenizer, params):
        ...
        delta = DeltaMessage(
            ...
            attention_index_mapping=new_response.get("attention_index_mapping"),
        )
        choice_data = ChatCompletionResponseStreamChoice(
            index=0,
            delta=delta,
...
def generate_stream_cogvlm(model: AutoModelForCausalLM, tokenizer: AutoTokenizer, params: dict):
    ...
    # === Step 7: Set up vision pruning if params provided ===
    _pruning_active = False
    _pruning_mapping = None
    api = params.get("active_patch_indices")
    rpg = params.get("room_patch_groups")
    # PruMerge already performs token reduction inside the vision tower.
    # Do not stack Step7 room/patch pruning on top of it.
    if prumerge_enabled() and (api is not None or rpg is not None):
        logger.warning("[Step7] Ignoring active_patch_indices/room_patch_groups because PruMerge is enabled")
        api, rpg = None, None
    logger.info(f"[Step7-DEBUG] api type={type(api)}, rpg type={type(rpg)}, "
                f"api is None={api is None}, rpg is None={rpg is None}, "
                f"api len={len(api) if api else 0}, rpg len={len(rpg) if rpg else 0}")
    if api is not None and rpg is not None and len(api) > 0:
        set_pruning_params(api, rpg)
        _pruning_active = True
        _pruning_mapping = get_pruning_state()["index_mapping"]
        logger.info(f"[Step7] Vision pruning active: R={_pruning_mapping['R']}, K={_pruning_mapping['K']}")
    ...
    if not params['return_string_probabilities']:
        ...
        generated_text = ""
        with torch.no_grad():
            _seq_len = model_inputs["input_ids"].shape[1]
            logger.info(f"[Timing] model.generate START | seq_len={_seq_len} | pruning={'YES R='+str(_pruning_mapping['R'])+' K='+str(_pruning_mapping['K']) if _pruning_active else 'NO'}")
            _t0 = time.time()
        ...
        _t1 = time.time()
        logger.info(f"[Timing] model.generate DONE | {(_t1-_t0)*1000:.1f}ms | pruning={'YES' if _pruning_active else 'NO'}")

        # Step 7: Clear pruning state after generation
        if _pruning_active:
            clear_pruning_params()
        ...
        # --- Step 5: Set up attention hooks before generation ---
        a_patch = None
        # PruMerge changes the vision-token count (~300, not 1600), so the
        # hardcoded-1600 attention extraction below would return a misaligned
        # vector. Skip it entirely when PruMerge is active (the baseline does
        # not need attention DoD anyway).
        if attn_hooks is not None and not prumerge_enabled():
            input_ids_list = model_inputs["input_ids"][0].tolist()
            BOI_TOKEN_ID = 151339
            if BOI_TOKEN_ID in input_ids_list:
                boi_pos = input_ids_list.index(BOI_TOKEN_ID)
                if _pruning_active and _pruning_mapping is not None:
                    n_vision = _pruning_mapping["R"] + _pruning_mapping["K"]
                    attn_hooks.set_image_token_range(boi_pos, n_vision, pruning_index_mapping=_pruning_mapping)
                else:
                    attn_hooks.set_image_token_range(boi_pos, 1600)
                attn_hooks.enable()
            else:
                logger.warning("[Step5] boi_token not found in input_ids, skipping attention extraction")
        ...
        with torch.no_grad():
            ...
            # --- Step 5: Extract attention weights after generation ---
            if attn_hooks is not None and not prumerge_enabled():
                a_patch = attn_hooks.compute_attention_weights()
                attn_hooks.disable()
            ...
            for next_text in streamer:
                ...
                yield {
                    ...
                    "attention_weights": a_patch,
                    "attention_index_mapping": _pruning_mapping,
                }
        # Step 7: Clear pruning state after generation
        if _pruning_active:
            clear_pruning_params()
...
if __name__ == "__main__":
    ...
    # Fix for ChatGLMConfig missing num_hidden_layers attribute
    ...
    # Step 5: Initialize attention extraction hooks on last 4 layers
    attn_hooks = AttentionCaptureHooks(model, layer_indices=[36, 37, 38, 39], top_m_heads=4)

    # Step 7: Install vision pruning hooks and monkey-patches
    install_pruning(model)

    # FastV ablation baseline (env-gated; inert unless PRUNE_METHOD=fastv).
    # Self-contained: no client/main.py changes needed. See fastv_pruning.py.
    configure_fastv_from_env()
    install_fastv(model)
    logger.info(f"[startup] {fastv_status()}")

    # PruMerge ablation baseline (env-gated; inert unless PRUNE_METHOD=prumerge).
    # Self-contained: no client/main.py changes needed. See prumerge_pruning.py.
    configure_prumerge_from_env()
    install_prumerge(model)
    logger.info(f"[startup] {prumerge_status()}")

    import argparse as _ap
    _p = _ap.ArgumentParser()
    _p.add_argument('--port', type=int, default=31511)
    _a = _p.parse_args()
    uvicorn.run(app, host='127.0.0.1', port=_a.port, workers=1)
