...
class CogVLM2:
    ...
    def create_chat_completion(self, model, messages, temperature=0.8, max_tokens=2048, top_p=0.8, use_stream=False,
                              active_patch_indices=None, room_patch_groups=None):
        """
        ...
        Args:
            ...
            active_patch_indices (list): Step 7 — list of active patch indices for pruning.
            room_patch_groups (dict): Step 7 — dict mapping room_id to patch index lists.
        ...
        # Step 7: Add pruning parameters if provided
        if active_patch_indices is not None:
            data["active_patch_indices"] = active_patch_indices
        if room_patch_groups is not None:
            data["room_patch_groups"] = room_patch_groups

        # Step 7 debug: log if pruning params are being sent
        if data.get("active_patch_indices"):
            import logging
            logging.info(f"[VLM-CLIENT] Sending pruning: {len(data['active_patch_indices'])} patches, "
                         f"{len(data.get('room_patch_groups', {}))} rooms, "
                         f"keys in data: {[k for k in data.keys()]}")
                ...
                for line in response.iter_lines():
                    if line:
                        ...
                        try:
                            ...
                            attention_weights = response_json.get("choices", [{}])[0].get("delta", {}).get("attention_weights", None)
                            attn_mapping = response_json.get("choices", [{}])[0].get("delta", {}).get("attention_index_mapping", None)
                            # logging.info(f"VLM output: {content}")
                        except:
                            print("Special Token:", decoded_line)
                return scores, content, attention_weights, attn_mapping
            else:
                # 处理非流式响应
                decoded_line = response.json()
                msg = decoded_line.get("choices", [{}])[0].get("message", "")
                content = msg.get("content", "")
                scores = msg.get("scores", "")
                attention_weights = msg.get("attention_weights", None)
                attn_mapping = msg.get("attention_index_mapping", None)
                return scores, content, attention_weights, attn_mapping
        else:
    ...
    def simple_image_chat(self, User_Prompt, return_string_probabilities=None, use_stream=False, img=None,
        ...
        Args:
            ...
            active_patch_indices (list): Step 7 — active patch indices for pruning.
            room_patch_groups (dict): Step 7 — room-to-patches mapping for pruning.
        ...
        scores, pred, attention_weights, attn_mapping = self.create_chat_completion(
            "cogvlm2", messages=messages, use_stream=use_stream,
            active_patch_indices=active_patch_indices, room_patch_groups=room_patch_groups)

        return scores, pred, attention_weights, attn_mapping
    ...
    def CoT2(self, User_Prompt1, User_Prompt2, cot_pred1, return_string_probabilities=None, use_stream=False, img=None):
        ...
        scores, pred, attention_weights, attn_mapping = self.create_chat_completion("cogvlm2", messages=messages, use_stream=use_stream)
