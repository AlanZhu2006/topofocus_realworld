...
class LLM_Agent_GT(Agent):
    def __init__(self, args, config_env, agent_id, device) -> None:
        ...
        self.red_sem_pred = load_rednet(
            self.device, ckpt='rednet_semmap_mp3d_40.pth', 
        ...
        # Wall mask for room segmentation
        self.wall_mask = None
        # Accumulated wall mask across all steps
        self.accumulated_wall_mask = torch.zeros(self.map_size, self.map_size).float().to(self.device)
    ...
    def mapping(self, observations: Observations):
        ...
        points, self.local_map, self.local_map_stair, self.local_pose, self.wall_mask = \
        ...
        # Accumulate wall mask over time (take max to preserve all detected walls)
        if self.wall_mask is not None:
            self.accumulated_wall_mask = torch.max(self.accumulated_wall_mask, self.wall_mask)
    ...
    def _preprocess_obs_rednet(self, obs, semantic, cur_mapping, use_seg=True):
        ...
        if 'objectnav_hm3d' in args.task_config:
            ...
            self._assign_gt_semantics(
                semantic, cur_mapping, sem_seg_pred_gt, dataset="hm3d",
            )
            ...
            self._assign_gt_semantics(
                semantic, cur_mapping, sem_seg_pred_gt, dataset="mp3d",
            )
    ...
    def init_map_and_pose(self):
        ...
        # Reset accumulated wall mask for new episode
        self.accumulated_wall_mask.fill_(0.)
