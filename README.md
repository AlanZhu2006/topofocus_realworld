...
- Hub 的协议、spool、单机语义映射、前沿/VLM 决策、Foxglove relay、TinyNav 原生 occupancy 适配器及 fail-closed 守门器已实现；本机测试基线为 142 项通过。
...
- Foxglove 可显示独立地图和已标定融合图；原生 TinyNav occupancy 必须保持自己的 frame，frame 不一致时 relay 会拒绝融合。
